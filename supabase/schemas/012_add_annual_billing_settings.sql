...
CREATE TABLE IF NOT EXISTS public.enterprise_billing_settings_history (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  enterprise_id UUID NOT NULL REFERENCES public.enterprises(id) ON DELETE CASCADE,
  old_annual_discount_percent NUMERIC,
  new_annual_discount_percent NUMERIC NOT NULL,
  changed_by UUID REFERENCES auth.users(id),
  changed_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE OR REPLACE FUNCTION public.log_enterprise_billing_settings_update()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.annual_discount_percent IS DISTINCT FROM OLD.annual_discount_percent THEN
    INSERT INTO public.enterprise_billing_settings_history (
      enterprise_id,
      old_annual_discount_percent,
      new_annual_discount_percent,
...
DROP TRIGGER IF EXISTS enterprise_billing_settings_log_update ON public.enterprise_billing_settings;
CREATE TRIGGER enterprise_billing_settings_log_update
BEFORE UPDATE ON public.enterprise_billing_settings
FOR EACH ROW
EXECUTE FUNCTION public.log_enterprise_billing_settings_update();

ALTER TABLE public.enterprise_billing_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.enterprise_billing_settings_history ENABLE ROW LEVEL SECURITY;

CREATE POLICY "enterprise_billing_settings_select_own"
ON public.enterprise_billing_settings
FOR SELECT
USING (
  enterprise_id IN (
...
CREATE POLICY "enterprise_billing_settings_insert_own"
ON public.enterprise_billing_settings
FOR INSERT
WITH CHECK (
  enterprise_id IN (
...
CREATE POLICY "enterprise_billing_settings_update_own"
ON public.enterprise_billing_settings
FOR UPDATE
USING (
  enterprise_id IN (
...
CREATE POLICY "enterprise_billing_settings_history_select_own"
ON public.enterprise_billing_settings_history
FOR SELECT
USING (
  enterprise_id IN (
...
CREATE POLICY "enterprise_billing_settings_history_insert_own"
ON public.enterprise_billing_settings_history
FOR INSERT
WITH CHECK (
  enterprise_id IN (
...
CREATE TABLE IF NOT EXISTS public.subscription_checkout_sessions (
  ...
  enterprise_id UUID NOT NULL REFERENCES public.enterprises(id) ON DELETE CASCADE,
  plan_id TEXT NOT NULL REFERENCES public.subscription_plans(id),
  billing_interval subscription_billing_interval_enum NOT NULL,
  currency TEXT NOT NULL DEFAULT 'XAF',
  monthly_price NUMERIC NOT NULL,
  annual_price_before NUMERIC,
  annual_discount_percent NUMERIC,
  annual_price_after NUMERIC,
...
CREATE POLICY "subscription_checkout_sessions_select_own"
ON public.subscription_checkout_sessions
FOR SELECT
USING (
  enterprise_id IN (
...
CREATE POLICY "subscription_checkout_sessions_insert_own"
ON public.subscription_checkout_sessions
FOR INSERT
WITH CHECK (
  enterprise_id IN (
